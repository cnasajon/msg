...
### folders
`id`, `organization_id`, `nome`, `descricao`, `timezone`, `telegram_chat_id`, `telegram_bot_token_cifrado` (**nulo por padrão** — sobreposição opcional; quando nulo, usa o bot global da variável de ambiente), `tipo_de_lista` (fila | data), `ao_esgotar` (parar_notificar | reiniciar), `alerta_de_fila_curta_ativo` (booleano), `alertar_abaixo_de` (inteiro), `ativa`, `criada_em`

**Participação em organizações.** A tabela `user_organizations (user_id, organization_id)` diz de quais organizações cada pessoa participa — substitui o antigo campo único `users.organization_id`. Quem participa de mais de uma escolhe no cabeçalho qual está operando, e o isolamento vale uma de cada vez, pela organização em vigor na sessão. A organização em vigor só é aceita se constar da participação: identificador de sessão é coisa que se tenta forjar. Conceder participação é exclusivo do superadmin — dar isso ao admin exigiria que ele enxergasse todas as organizações e lhe permitiria incluir a si mesmo em qualquer uma.

**Tipo de lista.** `fila` é o padrão: o slot leva o próximo texto ainda não publicado, na ordem. `data` seleciona pela data do slot — cada texto carrega um dia/mês/ano com curinga, descrito em `texts`. Em `data` são **duas condições simultâneas**: a data do slot casa com o padrão do texto **e** o dia da semana está marcado no agendamento. `*/09/*` com apenas domingo marcado sai nos domingos de setembro. Sem dia da semana marcado não existe slot, e nada é publicado. `ao_esgotar` e `alertar_abaixo_de` só valem para `fila`, porque em lista por data não há fila que acabe. `alerta_de_fila_curta_ativo` liga e desliga o aviso de fila curta e `alertar_abaixo_de` é o número de textos pendentes a partir do qual ele sai. Só a chave desliga — assim o número sobrevive ao silêncio e volta como estava. Os dois são editáveis na configuração da pasta e na tela de Alertas, linha a linha. Desligar silencia apenas esse aviso: falha de publicação, slot perdido e fila esgotada continuam valendo.
...
- **A lista ordena por qualquer coluna** ao clique no cabeçalho — primeiro clique ascendente, o seguinte descendente —, e a ordenação vive na URL junto dos filtros.
- **A coluna Ordem é a alça de arrastar**, com a posição e duas setas para telefone e teclado. Arrastar e ordenar por outra coluna não valem ao mesmo tempo: fora da ordem da fila, a posição de soltura não significa nada. As alças também somem com filtro ativo, em lista por data e para quem não pode reordenar; nesses casos a tela diz por quê, e a posição continua à mostra.
- **Incluir no meio da fila** pelo `+` da linha ou pela barra de seleção; o editor abre já posicionado, e nada é gravado antes de salvar.
- **A data de publicação é editável na tela do texto**, com a situação acompanhando: informar marca como `publicado` e grava uma publicação de origem `retroativa`, sem hora prevista; limpar devolve o texto ao fim da fila. A data é lida no fuso da pasta. As publicações que de fato saíram nunca são apagadas.
- **A largura é do conteúdo.** A tela existe para ler o texto, então tudo o mais cede espaço a ele: quando não há imagem, nada ocupa o lugar dela; a situação é um ícone colorido, não a palavra escrita; e abrir, publicar agora, arquivar, desarquivar e reenviar são ícones. Cada ícone leva o nome no `title`, que aparece ao passar o mouse, e no `aria-label`, que o leitor de tela anuncia — o botão fica estreito, não anônimo. A publicação imediata continua a um clique, e o erro continua escrito por extenso na legenda abaixo do texto, porque é a única situação que a data não denuncia sozinha.
- **A lista volta como estava.** Os seletores de pasta e de situação filtram no ato, sem passar pelo botão, e as duas escolhas ficam guardadas **no usuário** — quem trabalha numa pasta passa o dia voltando a ela, e recomeçar sempre na primeira em ordem alfabética obriga a reescolher a cada visita. É preferência da pessoa, não estado da sessão: sobrevive ao logout e acompanha quem troca de máquina, como já acontece com a organização ativa. A pasta lembrada é procurada dentro do que o escopo devolveu, nunca consultada por fora dele; se ela sumiu, foi desativada ou ficou em outra organização, a tela abre na primeira disponível. A busca digitada **não** é lembrada — uma caixa de busca preenchida de ontem esconde textos sem dizer por quê.
